...
export async function processData(json: DocumentData) {
  try {
    ...
    if (config.pageBackgrounds && json.doc?.bgColor) {
      page.backgrounds = [
        { type: "SOLID", color: parseColor(json.doc.bgColor) },
      ];
    }
    ...
    // Process the main frame (#NOTE: using the parent height instead frame size)
    if (json.frame) {
      console.log("Frame dimensions from JSON:", {
        width: json.frame.width,
        height: json.frame.height,
      });
      await createNode(
        { ...json.frame, height: json.height, width: json.width },
        parentNode,
        json.frame.x || 0,
        json.frame.y || 0
      );
